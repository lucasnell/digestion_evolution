#This code defines a function 'corp' that is largely the same as Tony's corphylo function, except that it is parameterized with the trait correlation matrix rather than the trait covariance matrix. This made it easier to calculate confidence intervals on the estimated correlation (using Fisher information)

#The output is the estimated trait correlation with confidence intervals; plus the trait standard deviations
...
#####All of the above is the same as the original 'corphylo' 
  ...
  tmax.SA = 1
  constrain.d = F
  #opt <- optim(fn = corphylo.LL, par = par, XX = XX, UU = UU, MM = MM, tau = tau, Vphy = Vphy, verbose = F, constrain.d = constrain.d, REML = T, method = "SANN", control = list(maxit = maxit.SA,temp = temp.SA, tmax = tmax.SA, reltol = reltol), hessian=T)
  #par <- opt$par
  opt <- optim(fn = corphylo.LL, par = par, XX = XX, UU = UU, MM = MM, tau = tau, Vphy = Vphy, verbose = F, constrain.d = constrain.d, REML = T, method = "Nelder-Mead", control = list(maxit = maxit.NM,                                                                                               reltol = reltol), hessian=T)
