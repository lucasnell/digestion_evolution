...
summ_df <- read_csv('output/models_summaries.csv', col_types = 'ccccdddd') %>%
    ...
    mutate(model = ifelse(phy_model == 'OU', 'corphylo', 'phylolm'),
           P = ifelse(X %in% c('lambda', 'd', 'alpha'), "–", P),
...
We used jackknifing to determine how influential each point is to the `log_mass`
and `clade` estimates.
Figure \ref{fig:results-influence-plots} shows that one species (*Artibeus lituratus*) has
...
```{r results-influence-plots, fig.height = 3, fig.width = 7, fig.cap = "Plot of influence values for the regression of intestinal length on clade and body mass. Point sizes are proportional to their influence (normalized to range from 0 to 1) on the maximum likelihood estimate for clade, body mass, and intercept. Facet labels indicate which parameter influence-values are pertaining to, and color indicates clade. Both axes are on the log scale."}
# Adding morphometric data to the jackknife data frame
jack_df <- dplyr::left_join(int_jack, get_df('spp'), by = 'species') %>% 
    group_by(estimate) %>% 
    mutate(influence = abs(influence) / max(abs(influence))) %>% 
    ungroup() %>% 
    select(diet, clade, species, log_mass, log_intestinal_length, 
           estimate, influence) %>% 
...
jack_df %>%
    ...
    facet_grid(~ estimate) + 
    scale_size_area(breaks = c(0.5, 1.0), max_size = 4) + 
...
Tables \ref{tab:results-clear-sef-kable}, \ref{tab:results-clear-ed-kable}, and
\ref{tab:results-absorp-te-kable}
refer to `corphylo` estimates for the correlations between clearance and SEF, 
clearance and enterocyte density, and
between fractional absorption and total enterocytes, respectively.
Each table includes the Pearson correlation coefficient ($r$) between the two traits 
...
filter(summ_df, model == 'corphylo')$data[[1]][4:6,] %>% 
    ...
    select(-phy_model, -Y) %>% 
    pretty_df(., caption = str_c('\\texttt{corphylo} model summary for the ',
                                 'correlation between clearance and enterocyte density. ',
                                 '$r$ indicates the correlation and $d_X$ indicates ',
                                 'the phylogenetic signal from an Ornstein-Uhlenbeck ',
                                 'process for variable $X$. ',
                                 'Both variables were log-transformed for this ',
...
filter(summ_df, model == 'corphylo')$data[[1]][7:9,] %>% 
    ...
    select(-phy_model, -Y) %>% 
    pretty_df(., caption = str_c('\\texttt{corphylo} model summary for the ',
                                 'correlation between fractional absorption and ',
                                 'total enterocytes. ',
                                 '$r$ indicates the correlation and $d_X$ indicates ',
                                 'the phylogenetic signal from an Ornstein-Uhlenbeck ',
                                 'process for variable $X$. ',
...
estimate.
Figure \ref{fig:results-influence-plot-corphylo} shows that one species 
...
cp_jack <- jack_corphylo(cp_mods$clear_sef, mean_df = clear_df, se_df = clear_se_df,
                         phy = clear_tr, par_names = c('log_sef', 'log_clear'))
```



```{r results-influence-plot-corphylo, fig.height = 3, fig.width = 3.5, fig.cap = "Plot of absolute influence values for the correlation between clearance and SEF. Point sizes are proportional to their influence on the maximum likelihood estimate of Pearson's r. Color indicates clade, and both axes are on the log scale."}
# Adding morphometric data to the jackknife data frame
cor_jack_df <- dplyr::left_join(cp_jack, get_df('clear'), by = 'species') %>% 
    mutate(influence = abs(influence)) %>% 
...
cor_jack_df %>%
    ...
    ggplot(aes(log_sef, log_clear, color = clade, size = influence)) +
    geom_point() +
    scale_x_continuous("Surface enlagement factor (SEF)", 
                       trans = 'log', breaks = 8 * 1.5^(0:2)) +
    scale_y_continuous(bquote(.("L-arabinose clearance (\u03BC") * 
                                  l ~ min^{-1} ~ cm^{-2} * ")"),
                       trans = 'log', breaks = 1 * 3^(0:2)) +
    scale_size_area(breaks = c(0.1, 0.2), max_size = 4) + 
    scale_color_manual(values = c('#9ecae1', '#de2d26')) +
    theme(legend.title = element_text(size = 10))
```
...
```{r results-influence-firstrun-corphylo}
influence_og <- cp_mods$clear_sef
first_run_df <- data_frame(parameter = 'r',
                           estimate = influence_og$cor.matrix[1,2],
                           lower = quantile(influence_og$bootstrap[,'r12'], 0.025),
                           upper = quantile(influence_og$bootstrap[,'r12'], 0.975))
```
...
rerun_df <- data_frame(parameter = 'r',
                       estimate = influence_rerun$cor.matrix[1,2],
                       lower = quantile(influence_rerun$bootstrap[,'r12'], 0.025),
                       upper = quantile(influence_rerun$bootstrap[,'r12'], 0.975))
list(first_run_df, rerun_df) %>% 
    bind_rows() %>% 
    mutate(`influential point` = c('present', 'absent')) %>% 
    select(-parameter) %>% 
    pretty_df(., caption = str_c('\\texttt{corphylo} estimates for the Pearson ',
                                 'correlation coefficient between log(clearance) and ',
                                 'log(SEF), with and  without influential point ',
