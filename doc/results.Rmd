...
For column $j$ from 1 to $p$, I did the following:

\begin{equation}
\label{eq:jackknife}
\begin{split}
    \mathbf{\hat{B}}_j &= \mathbf{B}_j - \mathbf{b}_{(0)j} \\
    \mathbf{\tilde{B}}_j &= \left| \frac{ \mathbf{\hat{B}}_j }{ 
        \sigma_{\hat{B}_j} } \right|
\end{split}
\end{equation}
...
The output $n$ by $p$ matrix, $\mathbf{\tilde{B}}$, presents absolute influence 
values scaled to have a standard deviation of one.
...
estimate.
See \ref{eq:jackknife} for how I calculated absolute influence values.
