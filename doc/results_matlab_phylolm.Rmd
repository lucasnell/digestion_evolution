...
one_param <- function(.j, .lines, .X, .Y, .pos) {
    str <- paste0('b', .j, ' = ', collapse = '')
    repl_str <- paste0('\\bb', .j, '\\b|%|=| |\\)', collapse = '')
    values <- .lines %>%
...
parse_output <- function(fn, .pos = NA) {
    if (grepl('.m$', fn)) {
        ...
        out_df <- read_csv(fn, col_types = 'cccddd') %>% 
            mutate(type = 'r', X = ifelse(X == 'taxonBat', 'taxon', X))
    }
    out_df <- out_df %>% 
...
summ_df <- map2_dfr(
    ...
    filter(!is.na(value_matlab)) %>% 
    mutate(R = sprintf('%.4g [%.4g, %.4g]', value_r, lower_r, upper_r),
           MATLAB = sprintf('%.4g [%.4g, %.4g]', value_matlab, lower_matlab, 
                            upper_matlab)) %>% 
    arrange(pos, Y, X) %>% 
    select(pos, Y, X, R, MATLAB) %>% 
    nest(-pos)
```
