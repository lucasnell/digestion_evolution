...
Xmat <- Xmat[clear_tr$tip.label,]


MEmat <- cbind(clear_se_df$log_sef, clear_se_df$log_clear)
rownames(MEmat) <- clear_se_df$species
MEmat <- MEmat[clear_tr$tip.label,]

Umat <- list(NULL, cbind(clear_df$log_mass))
rownames(Umat[[2]]) <- rownames(Xmat)
Umat[[2]] <- as.matrix(Umat[[2]][clear_tr$tip.label,])

# clear_sef_cor <- corp(Xmat, phy = clear_tr, SeM = MEmat) #, U = Umat)

corphylo(Xmat, phy = clear_tr, SeM = MEmat)
...
clear_ed_cor <- corp(Xmat, phy = clear_ed_tr, SeM = MEmat, U = Umat)

corp(Xmat, phy = clear_ed_tr, SeM = MEmat, U = Umat)
corphylo(Xmat, phy = clear_ed_tr, SeM = MEmat, U = Umat)
