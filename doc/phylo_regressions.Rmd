...
> The `spp_df` data frame is used for both this analysis and `Morphometrics` on `Clade`.
> The `tr` tree is used for this analysis, `Morphometrics` on `Clade`, and
...
```{r corphylo_under_hood, echo = FALSE}
# Thes "*_under_hood" chunks are to output summaries from the corphylo_cpp fits
...
pval(clear_sef)[1]
```
```{r clear_sef_under_hood, echo = FALSE}
pval(clear_sef)[1]
```
...
pval(clear_ed)[1]
```
```{r clear_ed_under_hood, echo = FALSE}
pval(clear_ed)[1]
```
...
pval(absorp_te)[1]
```
```{r absorp_te_under_hood, echo = FALSE}
pval(absorp_te)[1]
```
