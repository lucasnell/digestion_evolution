...
The `R/get_data.R` file provides functions to retrieve morphometric, clearance, 
and absorption data. See `tidy_csvs.md` for more info.
...
Retrieve data, change factors to binary variable(s), remove unnecessary parameters,
and write output for each segment type:
...
The same tree is used for these as for analyses by species only, so the file
`matlab/data/spp_vcv.txt` can be used.
