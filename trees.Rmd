...
# str(rod_tr)
# length(rod_tr$edge.length[is.na(rod_tr$edge.length)])
# 
# na_edges <- which(is.na(rod_tr$edge.length))
# 
# rod_tr$tip.label[na_edges[2]]
# rod_tr$edge.length[na_edges[2] + c(-1,0,1)]
# 
# na_nodes <- c(rod_tr$edge[na_edges,], recursive = TRUE) %>% unique
# 
# na_tips <- sapply(na_nodes, function(x) extract.clade(rod_tr, node = x)$tip.labels)
# 
# rod_tr$edge[1:4,]
# rod_tr$edge.length[1:4]
# 
# 1417 + 2260
# 
# dichotomousphylogeny <- multi2di(phylogeny, random = FALSE)
# bin_rod <- multi2di(rod_tr, random = TRUE)
# 
# rod_tr %>% str
# 
# ran <- rtree(n = 5)
...
```{r, eval = FALSE}
font_size <- 0.05
pdf('rod_tree.pdf', width = 6, height = 18)
...
If you want to make a pdf of this tree that's (somewhat) readable:
```{r, eval = FALSE}
pdf('bat_tree.pdf', width = 6, height = 12)
